...
def loai_bo_khoang_trang(chuoi_goc):
    """Hàm tự cài đặt thay thế cho .strip()"""
    if chuoi_goc == "":
        return ""
...
def chuoi_sang_so(chuoi):
    """Hàm tự phân tích chuỗi số nguyên."""
    so_nguyen = 0
    i = 0
    chieu_dai = 0
    for c in chuoi:
...
def kiem_tra_dinh_dang_gio(chuoi_gio):
    """
    Kiểm tra chuỗi giờ đúng định dạng HH:MM.
    """
    chieu_dai = 0
    for c in chuoi_gio:
        chieu_dai += 1
...
def chuan_hoa_ten(chuoi):
    ...
    while i < chieu_dai:
        ...
        # Nếu là khoảng trắng, giữ nguyên và đánh dấu từ tiếp theo bắt đầu
        if ky_tu == ' ':
            ket_qua += ky_tu
            dau_tu = True
            i += 1
        ...
        # Nếu là chữ cái đầu tiên của từ
        if dau_tu:
            # Nếu là chữ thường (a-z: 97-122), chuyển thành in hoa (-32)
            if 97 <= ma_ascii <= 122:
                ket_qua += chr(ma_ascii - 32)
            else:
                ket_qua += ky_tu
            dau_tu = False
        # Nếu là các chữ cái tiếp theo trong từ
        else:
